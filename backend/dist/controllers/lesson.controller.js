...
async function completeLesson(req, res) {
    ...
    try {
        ...
        // Find progress
        let progress = await prisma_1.default.lessonProgress.findUnique({
            where: {
                userId_lessonId: { userId, lessonId },
            },
        });
        if (!progress) {
            ...
            progress = await prisma_1.default.lessonProgress.create({
                data: {
                    userId,
                    lessonId,
                    courseId: lesson.courseId,
        ...
        const completedProgress = await prisma_1.default.lessonProgress.update({
